...
class DianPingSpider(object):
    ...
    def __init__(self, url='http://www.dianping.com/shop/586341'):
        ...
        self.rw = self.get_bis(css_html, charcb)

    def get_index_html(self, url):
        '''
        获取初始网页
        '''
        url='http://www.dianping.com/shop/59441690'
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.92 Safari/537.36'}
        ip={'http':'http://101.4.136.34:80'}
        resp = requests.get(url=url, headers=headers,proxies=ip)
        html = resp.text
        print(html)
        return html
    ...
    # def get_rw(self, css_html, numcb):
    #     '''
    ...
    def get_rw(self, css_html, charcb):
        '''
        获取bi开头的class属性真正显示的汉字
        '''
        # 提取相应的svg文件的url
        regex = re.compile(r'\[class\^="%s-"\][\s\S]*?url\((.*?)\)' % charcb)
        svg_url = re.search(regex, css_html)
        if svg_url.startswith('//'):
            svg_url = 'http:' + svg_url
        # 得到svg的内容
        resp = requests.get(svg_url)
